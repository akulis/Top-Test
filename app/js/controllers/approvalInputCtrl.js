four51.app.controller('ApprovalInputCtrl', [
  '$scope',
  '$location',
  '$rootScope',
  'Order',
  'Address',
  function($scope, $location, $rootScope, Order, Address) {
    $scope.approveOrder = function() {
      $scope.loadingIndicator = true;
      $scope.approveClicked = true;
      Order.approve($scope.order, function(data) {
        $scope.order = data;
    ...
    $scope.declineOrder = function() {
      $scope.loadingIndicator = true;
      Order.decline($scope.order, function(data) {
        $scope.order = data;
        $scope.loadingIndicator = false;
      }, function(ex) {
        $scope.loadingIndicator = false;
