four51.app.factory('ProductDisplayService', [
  '$sce',
  '$451',
  'Variant',
  'Product',
  function($sce, $451, Variant, Product) {
    function calcTotal(lineItem) {
      ...
      var addToMarkups = function(spec) {
        var otherMarkup;
        if (spec.AllowOtherValue && spec.OtherValueMarkup > 0 && (spec.isOtherSelected || (spec.Value && !spec.SelectedOptionID))) 
      ...
      if (variant) 
        angular.forEach(variant.Specs, addToMarkups);
      ...
      var debugLineTotal = "line total debug:\rquantity:" + lineItem.Quantity + " & " + "amount added per quantity:" + amountPerQty + " & " + "fixed ammount per line added:" + fixedAddPerLine + " & " + "percentage added to qty*unitprice:" + percentagePerLine + " & " + //"'other value' markup:" + otherValueMarkup + " & " +
    ...
    function productViewScope(scope) {
      ...
      scope.specChanged = function(spec) {
        ...
        if (spec.DefinesVariant) {
          ...
          $451.filter(scope.LineItem.Specs, {
            Property: 'DefinesVariant',
            Value: true
          }, function(item) {
      ...
      scope.inventoryDisplay = function(product, variant) {
        var qa = product.IsVariantLevelInventory
          ? variant
          : product;
        if (qa) 
          return qa.QuantityAvailable > 0
    ...
    function newLineItemScope(scope) {
      ...
      scope.currentFirstVariant = scope.settings
        ? (scope.settings.currentPage * scope.settings.pageSize) - scope.settings.pageSize
        : 0;

      scope.allowAddFromVariantList = ((scope.LineItem.Product.Type == 'VariableText' && scope.LineItem.Product.ShowSpecsWithVariantList) || (scope.LineItem.Product.Type == 'Static' && scope.LineItem.Product.ShowSpecsWithVariantList && hasAddToOrderSpecs) || !hasAddToOrderSpecs) && !scope.LineItem.Variant && scope.LineItem.Product.Variants && scope.LineItem.Product.Variants.length > 0 && ((scope.LineItem.Product.Variants && scope.LineItem.Product.Variants.length > 0) || scope.LineItem.Product.Type == 'VariableText')
      ...
      if (scope.LineItem.Variant) {
        ...
        scope.LineItem.PriceSchedule = variantHasPriceSchedule(
          scope.LineItem.Product, scope.currentOrder
          ? scope.currentOrder.Type + 'PriceSchedule'
              ...
              Variant.get({
                ...
                scope.variantLineItems[v.InteropID] = {
                  PriceSchedule: (
                    ...
                    : (v.StandardPriceSchedule || v.ReplenishmentPriceSchedule)) || (
                    scope.currentOrder
                    ? p[scope.currentOrder.Type + 'PriceSchedule']
                    : (v.StandardPriceSchedule || v.ReplenishmentPriceSchedule || scope.LineItem.PriceSchedule)),
                  Product: p,
                  Variant: data,
      ...
      function allowAddToOrder() {
        return (
          scope.currentOrder
          ? canAddToOrderType(scope.currentOrder.Type)
          : true) && (scope.allowAddFromVariantList || (scope.LineItem.Variant || (scope.LineItem.Product.VariantCount == 0 && scope.LineItem.Product.Type != 'VariableText')));
      }
      ...
      function canAddToOrderType(type) {
        ...
        if (scope.variantLineItems && scope.LineItem.Product.Type == 'VariableText') {
          if (!scope.user.Permissions.contains(type + 'Order')) 
            return false;
          if (scope.currentOrder && scope.currentOrder.ID && scope.currentOrder.Type != type) 
            return false;
        ...
        if (scope.user) {
          return scope.user.Permissions.contains(type + 'Order') && scope.variantLineItems
            ? scope.variantLineItems[scope.LineItem.Product.Variants[scope.currentFirstVariant].InteropID].Variant[type + 'PriceSchedule'] != null
            : scope.LineItem.Product[type + 'PriceSchedule'] != null && (
              ...
              ? (
                scope.variantLineItems
                ? scope.variantLineItems[scope.LineItem.Product.Variants[scope.currentFirstVariant].InteropID].PriceSchedule.OrderType
                : scope.LineItem.PriceSchedule.OrderType) == scope.currentOrder.Type
              : true);
        }
      }
    ...
    function _getProductAndVariant(productInteropID, variantInteropID, callback, page, pagesize, searchTerm) {
      Product.clearCache().get(productInteropID, function(data) {
        ...
        if (variantInteropID) {
          Variant.get({
            VariantInteropID: variantInteropID,
            ProductInteropID: p.InteropID
          }, function(v) {
