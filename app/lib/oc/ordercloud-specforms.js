...
function ocselectionfield($451) {
  ...
  function OCSelectionFieldLink(scope, element, attr) {
    scope.changed = function() {
      ...
      // end reset
      scope.customfield.Value = this.item == null
        ? null
        : this.item.Value;
      scope.customfield.SelectedOptionID = this.item == null
        ? null
        : this.item.ID;
      if (this.item != null) 
    ...
    scope.item = {},
    scope.other = ''; // initialize the item variable to avoid checking for null

    scope.init = function() {
      var id = scope.customfield.Value != null
        ? scope.customfield.Options[scope.customfield.Options.length - 1].ID
        : scope.customfield.DefaultOptionID;
      var matched = false;
      angular.forEach(scope.customfield.Options, function(n, i) {
        ...
        if (scope.customfield.Value == null) {
          ...
          if (id != null) 
            scope.customfield.Value = $451.filter(scope.customfield.Options, {
              'Property': 'ID',
              'Value': id
            })[0].Value;
          matched = true;
      ...
      this.item = scope.customfield.Value != null
...
function occheckboxfield() {
  ...
  function template() {
    return ['<div class="checkbox">', '<label ng-class="{\'required\': customfield.Required}">{{label || customfield.Label || customfield.Name}}', '<input type="checkbox" ng-true-value="{{checked}}" ng-false-value="{{unchecked}}" ng-model="customfield.Value">', '</label>', '</div>'].join('');
  }
}
        ...
        function keydownEvent(e) {
          if (!input.prop("readonly")) {
            ...
            8 === k || 46 === k || iPhone && 127 === k
              ? (pos = input.caret(), begin = pos.begin, end = pos.end, end - begin === 0 && (
                begin = 46 !== k
                ? seekPrev(begin)
